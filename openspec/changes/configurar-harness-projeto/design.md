...
**Decisão:** adicionar um hook `PreToolUse` para a ferramenta Bash em `.claude/settings.json`, que casa comandos contendo `git push` e nega (`deny`) a execução incondicionalmente — inclusive quando o desenvolvedor pede isso na própria conversa —, com uma mensagem indicando que o push deve ser feito manualmente pelo desenvolvedor. `git commit` e `git merge` locais pelo agente NÃO são bloqueados por este hook.
**Motivo:** o desenvolvedor validou explicitamente que agentes de IA podem commitar e mesclar localmente (isso não afeta o repositório remoto nem terceiros), mas o push para o GitHub remoto deve ser sempre uma ação manual, revisada por ele. Um hook `PreToolUse` é o único ponto de controle técnico disponível para interceptar a ação antes que a ferramenta Bash a execute — diferente de um git hook (`pre-commit`/`pre-push`), que não consegue distinguir se o comando `git` foi disparado pelo agente ou diretamente pelo desenvolvedor no terminal. Alternativa descartada: bloquear também `git commit`/`git merge` (versão anterior deste design) — descartada porque o desenvolvedor corrigiu explicitamente que commits e merges locais são desejáveis como parte do fluxo de trabalho com o agente; apenas o push é a operação de escopo compartilhado que exige autorização manual.
**Limite conhecido:** o hook se aplica somente a comandos `git push` disparados pela ferramenta Bash do Claude Code; não impede o desenvolvedor de dar push manualmente no terminal (comportamento desejado) nem cobre outros agentes de IA fora do Claude Code, caso venham a ser usados no projeto.
...
- **O hook de bloqueio de push é uma configuração local do Claude Code (`.claude/settings.json`) e pode, em tese, ser removida ou sobrescrita por quem tiver acesso ao repositório** → Mitigação: versionar `.claude/settings.json` no repositório (não em `.claude/settings.local.json`, que normalmente é ignorado) para que a remoção apareça como uma alteração revisável em PR; ainda assim, o controle definitivo continua sendo o hábito do desenvolvedor de nunca pedir ao agente para dar push.
