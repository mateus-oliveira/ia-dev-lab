...
- [ ] 2.2 Verificar que `poetry run alembic downgrade -1` a partir da revisão inicial remove a tabela `users` corretamente, confirmando a reversão

## 3. Remoção da criação de schema ad-hoc

- [ ] 3.1 Remover a chamada a `init_db()` do `lifespan` em `src/player_modeling/api/app.py`; verificar que a API sobe normalmente com `poetry run uvicorn player_modeling.api.app:app` sem criar `db.sqlite3` automaticamente
- [ ] 3.2 Remover a função `init_db()` de `src/player_modeling/api/database.py` (schema passa a ser responsabilidade exclusiva das migrações); verificar com `mypy`/`ruff` que não há referências órfãs

## 4. Ajuste dos testes existentes

- [ ] 4.1 Atualizar `src/tests/player_modeling/api/test_database.py` e qualquer fixture que dependa de `init_db()` para aplicar o schema via Alembic (ex.: chamando `alembic.command.upgrade` programaticamente contra um banco temporário) em vez da função removida; verificar com `poetry run pytest src/tests/player_modeling/api/test_database.py`
- [ ] 4.2 Rodar a suíte completa e confirmar que nenhum outro teste (`test_register.py`, `test_login.py`, `test_protected_routes.py`, etc.) dependia implicitamente de `init_db()`; verificar com `poetry run pytest`
